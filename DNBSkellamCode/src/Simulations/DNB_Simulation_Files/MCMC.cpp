...
MCMC::MCMC(vector<double> vParam, string sType_in, int iSeedNo_in)
{
	if(vParam.size()==4 ||  vParam.size()==5 )
	{
		...
		string sSeedNumber;
		stringstream sSeedNumber_out; 
		sSeedNumber_out << iSeedNo;
		sSeedNumber = sSeedNumber_out.str();
 
		sType = sType_in + sSeedNumber;
	}
	else
	{
...
void MCMC::ExportSimulatedData2()
{
	string sPrefix;
...
/* **************************************************************** */
/* *********** THE MAIN ESTIMATION FUNCTION FOR SKELLAM *********** */
/* **************************************************************** */
void MCMC::EstimateSkellam( int iNumOfIter, int iBurnIn)
{
	...
	if (sInput.empty()) 
	{
		/* To speed up the computations we can start from the true DGP parameters for the simulation (no convergence issues). But different values can be set by uncommenting some examples below. */
		...
		sParam.dGamma[0]= dGammaTrue; //0.01; //0.3; //0.0;
	} 
	else
	{
...
/* ***************************************** */
/* *** Main loop for the MCMC iterations *** */
/* ***************************************** */		
	cout<<"start"<<endl;
...
/* ************************************************************ */
/* *********** THE MAIN ESTIMATION FUNCTION FOR DNB *********** */
/* ************************************************************ */
void MCMC::EstimateDNB(int iNumOfIter, int iBurnIn)
{
	...
	if (sInput.empty()) 
	{
		...
		sParam.dPhi[0]= dPhiTrue; //0.97; // 0.95;
		sParam.dSigma2[0]=dSigma2True; // 0.02; //0.01;
		sParam.dGamma[0]= dGammaTrue; //0.01; //0.3; //0;
	...
	sParam.dPriorGammaA[0]=1.7;
	sParam.dPriorGammaB[0]=10;
	sParam.dPriorNuA[0]= 15;
	sParam.dPriorNuB[0]=1.5;
	sParam.dPriorMuMean[0]=0;
	sParam.dPriorMuSigma2[0]=10;
	sParam.dPriorPhiA[0]=20;
	sParam.dPriorPhiB[0]=1.5;
	sParam.dPriorSigmaA[0]=2.5;
	sParam.dPriorSigmaB[0]=1/0.025;
...
//		DrawNuDiscreteDNB(sParam , iNumOfObs, gsl_random_num, 0.025,0.05);
		DrawNuInBlockDiscreteDNB(vData, sParam ,  iNumOfObs, gsl_random_num, 0.2,6); //3
